...
import base64
import Cython
import signalsDELV as signals          #For the threads creation 
import signal                          #Used to enable the key interrupt.
import cv2                             #Open CV librarie 
import pyzbar.pyzbar as pyzbar         #Decoding QR librarie 
from   pyzbar.pyzbar import ZBarSymbol #To define tyhe code type to be decode 
...
from   multiprocessing import Process  #Multiroceess
import Module.decodeModule  as decM    #To desencipt the QR information 
import Module.QrGpioModule  as LEDS    #To enable and play GPIOS connected
import Module.requestModule as Flow    #To make the request to the servers
import Module.PID_file_gen  as PID     #To get the number of the task thats doimg the process
import Module.SQL_Adq       as SQL     #To consum the database 

_logger = log.configure_logger('default')
_logger.info('Task : {}'.format(PID.ret_pid()))
#PID.save_PID()

try: 
  cameraCV = cv2.VideoCapture(0)
except:
  try : 
     cameraCV = cv2.VideoCapture(1)
  except: 
     _logger.critical('OJE000')
     print("Couldnt Connect to the Cammera")
     exit(0)
